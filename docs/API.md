...
| 字段 | 类型 | 含义 |
| --- | --- | --- |
| `inventoryNo` | String | 碳盘查编号 |
| `activityDataCoverage` | double | 活动数据覆盖率(%) |
| `factorCoverage` | double | 排放因子覆盖率(%) |
| `verificationPassRate` | double | 核证抽样通过率(%) |
| `forecastReductionRate` | double | 预测减排率(%) |
| `targetReductionRate` | double | 目标减排率(%) |
| `unresolvedAnomalies` | int | 未解决异常数 |
| `methodologyApproved` | boolean | 核算方法已审批 |
| `independentVerified` | boolean | 已完成独立核证 |
